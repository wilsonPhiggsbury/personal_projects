	...
	public class GameWindow extends MovieClip
	{
		...
		public function enterframe()
		{
			...
			else
			{
				...
				drawShieldLines();
			}var i;for (i=tt.length-1;i>=0;i--)
			{
				removeChild(tt[i]);
				tt.splice(i,1);
			}text(new Point(-50,0),String(onCorner!=null),tt);
			// test mark
		...
		function merge():void
		{
			...
			*/
			...
			if (inPolygon(test_pt,result_corners_1))
			{
				corners = result_corners_1;
				trace("Corners assigned to result 1");
			}
			else if (inPolygon(test_pt,result_corners_2))
			{
		...
		function inPolygon(point:Point,polygon:Array):Boolean
		{
			...
			else
			{
				...
				{
					if (between(polygon[i].coords.y,point.y,polygon[mod(i+1,polygon.length)].coords.y) && polygon[i].coords.x<point.x)
					{
						intersects++;
					}
				}
		...
		function drawShieldLines():void
		{
			...
			if (exploring)
			{
				...
				for (var i:int = 1; i < newCorners.length; i++)
				{
