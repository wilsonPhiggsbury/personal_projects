...
void main()
{
	...
	// output file
	char *filemode = "w";
	FILE *outFile = fopen("output.txt",filemode);
	...
	fprintf(outFile, "____________%d____________Profit: %d\n", n, head->pathStruct.profit);
	...
	do
	{
		...
		while(temp->pathStruct.path[i+1]!=ROWS)
		{
			...
			fprintf(outFile, "%d,",temp->pathStruct.path[i++]);
		}
		//printf("%d",temp->pathStruct.path[i]);
		fprintf(outFile, "%d",temp->pathStruct.path[i++]);
	...
	endClock = clock();
	printf("Whole process took %ld miliseconds.",endClock-startClock);
